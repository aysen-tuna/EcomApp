...
function getStockStatus(stock: number = 0) {
  if (stock <= 0)
    return { label: 'No stock', className: 'bg-red-600/15 text-red-700 dark:text-red-300' };
  if (stock < 5)
    return { label: 'Low stock', className: 'bg-amber-500/15 text-amber-700 dark:text-amber-300' };
  return {
    label: 'In stock',
    ...
    <div className="relative w-full max-w-[340px] sm:max-w-none rounded-xl border border-black/10 dark:border-white/10 bg-neutral-100/70 dark:bg-neutral-800/70 p-3 sm:p-4 shadow-sm transition hover:shadow-md flex flex-col">
      ...
      {user && (
        <Button
          ...
          onClick={() => {
            if (canAddToCart) addPriceToCart(p.stripePriceId, uid, maxStock);
          }}
        >
        ...
        {isAdmin ? (
          ...
          <>
            <div className="flex items-center gap-2 pt-1">
              ...
              <span
                className={`text-xs px-2 py-1 rounded-full ${getStockStatus(p.stock).className}`}
              >
        ...
        <div className="mt-auto pt-4 flex justify-end gap-2">
          {confirming ? (
            ...
            <>
              <Button
                asChild
                className="bg-neutral-100 text-black hover:bg-white dark:bg-neutral-300 dark:text-black dark:hover:bg-neutral-400"
              >
