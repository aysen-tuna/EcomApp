    ...
    async function run() {
      ...
      try {
        ...
        setValues({
          ...
          discountRate: data?.discount?.rate != null ? String(data.discount.rate) : '',
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      if (props.mode === 'create') {
        const { createProduct } = await import('@/app/actions/admin/products/new');
        await createProduct(user.uid, raw);
      } else {
        const { editProduct } = await import('@/app/actions/admin/products/edit');
